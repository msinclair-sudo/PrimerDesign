...
# MAFFT
# ---------------------------------------------------------------------------
def run_mafft(records: list[tuple[str, str]], adjust_direction: bool = False) -> list[tuple[str, str]]:
    """Run MAFFT alignment. Returns list of (name, aligned_seq) in input order.
    ...
    return [(name, aligned[name]) for name in [r[0] for r in records] if name in aligned]
...
def main():
    ...
    aligned = run_mafft(raw_records, adjust_direction=True)
    ...
    print("=" * 60)
    realigned = run_mafft(rotated_records)
